...
본 문서에서는 Server와 Agent 모두 Ubuntu 14.04를 사용한다고 가정한다. 또한 본 문서에서 Ambari Server는 기본적으로 사용하는 PostgreSQL 대신 MySQL을 사용한다고 가정한다. 본 문서에서 설명하지 않은 부분에 대한 내용이 필요할 경우 다음 웹페이지를 참조할 수 있다.
